...
class BankHolidayTestCase(unittest.TestCase):
    ...
    def get_bank_holidays_using_local_data(cls, **kwargs):
        with responses.RequestsMock() as rsps:
            rsps.add(rsps.GET,
                     ...
                     json=BankHolidays.load_backup_data())
    ...
    def assertExpectedFormat(self, holidays):  # noqa: N802
        ...
        for holiday in holidays:
            self.assertListEqual(sorted(holiday.keys()),
                                 expected_keys,
                                 msg='Unexpected or missing dictionary keys')
            if last_holiday:
                self.assertGreater(holiday['date'],
                                   last_holiday['date'],
    ...
    def test_holidays(self):
        ...
        holidays = list(
            bank_holidays.holidays(start=datetime.date(2012, 1, 1)))
    ...
    def test_holidays_for_division(self):
        ...
        self.assertEqual(len(holidays),
                         BACKUP_DATA_ENGLAND_AND_WALES_HOLIDAY_COUNT)
        self.assertExpectedFormat(holidays)
        holidays = list(
            bank_holidays.holidays(start=datetime.date(2012, 1, 1),
                                   division=BankHolidays.SCOTLAND))
        self.assertEqual(len(holidays), BACKUP_DATA_SCOTLAND_HOLIDAY_COUNT)
        self.assertExpectedFormat(holidays)
        self.assertIn('St Andrew’s Day',
                      map(lambda holiday: holiday['title'], holidays))
    ...
    def test_holidays_for_year(self):
        ...
        self.assertExpectedFormat(holidays)
        self.assertTrue(
            all(holiday['date'].year == 2017 for holiday in holidays))

    def test_holidays_for_division_and_year(self):
        bank_holidays = self.get_bank_holidays_using_local_data()
        holidays = bank_holidays.get_holidays(
            division=BankHolidays.NORTHERN_IRELAND, year=2016)
        self.assertEqual(len(holidays), 10)
        self.assertExpectedFormat(holidays)
        self.assertIn('Battle of the Boyne (Orangemen’s Day)',
                      map(lambda holiday: holiday['title'], holidays))
        self.assertTrue(
            all(holiday['date'].year == 2016 for holiday in holidays))

    def test_holiday_iterator(self):
        bank_holidays = self.get_bank_holidays_using_local_data()
        holidays_1 = [holiday for holiday in bank_holidays]
        holidays_2 = bank_holidays.get_holidays(
    ...
    def test_next_bank_holiday(self):
        ...
        self.assertEqual(
            bank_holidays.get_next_holiday(division=BankHolidays.SCOTLAND,
                                           date=second_january)['date'],
    ...
    def test_prev_bank_holiday(self):
        ...
        self.assertEqual(
            bank_holidays.get_prev_holiday(division=BankHolidays.SCOTLAND,
                                           date=fifth_january)['date'],
            datetime.date(2016, 1, 4))
    ...
    def test_is_holiday_check(self):
        ...
        self.assertTrue(
            bank_holidays.is_holiday(datetime.date(2016, 1, 4),
                                     division=BankHolidays.SCOTLAND))
    ...
    def test_next_work_day(self):
        ...
        self.assertEqual(
            bank_holidays.get_next_work_day(division=BankHolidays.SCOTLAND,
    ...
    def test_prev_work_day(self):
        ...
        self.assertEqual(
            ...
            datetime.date(2018, 1, 2))
        self.assertEqual(
            bank_holidays.get_prev_work_day(division=BankHolidays.SCOTLAND,
    ...
    def test_is_work_day(self):
        ...
        self.assertFalse(
            bank_holidays.is_work_day(datetime.date(2018, 1, 2),
                                      division=BankHolidays.SCOTLAND))

    def test_configuring_weekends(self):
        bank_holidays = self.get_bank_holidays_using_local_data(
            weekend={1, 5, 6})
        self.assertFalse(bank_holidays.is_work_day(datetime.date(2017, 12,
                                                                 19)))
