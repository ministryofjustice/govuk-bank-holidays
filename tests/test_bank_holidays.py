...
from govuk_bank_holidays.bank_holidays import BankHoliday, BankHolidays, Division
...
class BankHolidayTestCase(unittest.TestCase):
    @classmethod
    def get_bank_holidays_using_local_data(cls, **kwargs) -> BankHolidays:
        with responses.RequestsMock() as rsps:
            rsps.add(rsps.GET, BankHolidays.source_url, json=BankHolidays.load_backup_data())
            return BankHolidays(**kwargs)

    def assertExpectedFormat(self, holidays: typing.List[BankHoliday]):  # noqa: N802
        last_holiday = None
    ...
    def test_holidays_for_division(self):
        ...
        holidays = bank_holidays.get_holidays(division=Division.ENGLAND_AND_WALES)
        holidays_2018_to_2022 = [holiday for holiday in holidays if 2018 <= holiday.date.year <= 2022]
        ...
        holidays = bank_holidays.get_holidays(division=Division.SCOTLAND)
        holidays_2018_to_2022 = [holiday for holiday in holidays if 2018 <= holiday.date.year <= 2022]
    ...
    def test_holidays_for_division_and_year(self):
        bank_holidays = self.get_bank_holidays_using_local_data()
        holidays = bank_holidays.get_holidays(division=Division.NORTHERN_IRELAND, year=2016)
    ...
    def test_next_bank_holiday(self):
        ...
        self.assertEqual(
            bank_holidays.get_next_holiday(division=Division.SCOTLAND, date=second_january).date,
    ...
    def test_prev_bank_holiday(self):
        ...
        self.assertEqual(
            bank_holidays.get_prev_holiday(division=Division.SCOTLAND, date=fifth_january).date,
            datetime.date(2016, 1, 4)
        )
    ...
    def test_is_holiday_check(self):
        ...
        self.assertTrue(bank_holidays.is_holiday(datetime.date(2016, 1, 4), division=Division.SCOTLAND))
    ...
    def test_next_work_day(self):
        ...
        self.assertEqual(
            bank_holidays.get_next_work_day(division=Division.SCOTLAND, date=datetime.date(2017, 12, 30)),
    ...
    def test_prev_work_day(self):
        ...
        self.assertEqual(
            bank_holidays.get_prev_work_day(division=Division.SCOTLAND, date=datetime.date(2018, 1, 3)),
    ...
    def test_is_work_day(self):
        ...
        self.assertFalse(bank_holidays.is_work_day(datetime.date(2018, 1, 2), division=Division.SCOTLAND))
    ...
    def test_future_holiday_generators(self):
        ...
        generator = bank_holidays.holidays_after(division=Division.SCOTLAND, date=datetime.date(2016, 1, 2))
    ...
    def test_past_holiday_generators(self):
        ...
        generator = bank_holidays.holidays_before(division=Division.SCOTLAND, date=datetime.date(2016, 1, 5))
        self.assertEqual(next(generator).date, datetime.date(2016, 1, 4))
    ...
    def test_future_work_day_generators(self):
        ...
        generator = bank_holidays.work_days_after(division=Division.SCOTLAND, date=datetime.date(2017, 12, 30))
        self.assertSequenceEqual(
            list(itertools.islice(generator, 3)),
    ...
    def test_past_work_day_generators(self):
        ...
        self.assertEqual(next(generator), datetime.date(2017, 12, 27))
        generator = bank_holidays.work_days_before(division=Division.SCOTLAND, date=datetime.date(2018, 1, 3))
        self.assertEqual(next(generator), datetime.date(2017, 12, 29))
        self.assertEqual(next(generator), datetime.date(2017, 12, 28))
        self.assertEqual(next(generator), datetime.date(2017, 12, 27))
